...
/*
    ...
    params: {
        ...
        transform: {
            x,
            y,
            width,   // Width will depend on how many fields
            height,
            *minheight,
            scale   // Default 1
...
function createSvgBarPath(data, transform, config = {}) {
    var path = '';
    ...
    // Gap between end of numeral indicators at left and init of vertical graphic axis
    var _defNumGap = 2 * GAP2;
    var _defTextHeight = 15;

    // Numeral Serparator of reigh axis
    var _numSep = 2 * GAP2;
    // default yaxis info
    var y_scale = 100;
    var y_unit = 'm';
    const { x, y, width, height, scale } = transform;
    const { axisx, axisy, values, promedy } = data;
    const { grid, title } = config;
    if (x != undefined) { _X = x; }
    if (y != undefined) { _y = y; }
    ...
    if (data != undefined) {
        if (axisy != undefined) {const { scale, unit } = axisy;if( scale != undefined ) { y_scale = scale; }if( unit != undefined ) { y_unit = scale; }}

    }
    // Psition at init;
    path += 'M ' + (_x + _defNumGap) + ',' + _y + ' ';
    // draw vertical line
    path += 'l  0,' + _height + ' ';
    path += 'l ' + _width + ',0 ';
...
module.exports.createSvgBarPath = createSvgBarPath;